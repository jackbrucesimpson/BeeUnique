...
for VIDEO_FILENAME_PATH in $VIDEO_FILE_ARRAY; do
    python ../src/track.py $VIDEO_FILENAME_PATH $OUTPUT_DIRECTORY $EXPERIMENT_NAME $TRAINING $NUM_FRAMES_THREAD_QUEUE $NUM_FRAMES_BATCH_PROCESS $N_PROCESSES $CHUNKSIZE
done
